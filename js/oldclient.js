...
(function($) {
	...
	var App = this.App = Backbone.Router.extend({
		...
		initializeConnection: function() {
			if ((Config.origindomain !== Config.origindomain) && !Config.testclient) {
				// Handle *.psim.us.
		...
		focusRoom: function(id) {
			...
			if (this.curSideRoom !== room) {
				...
				this.updateLayout();
				if (this.curRoom.id === id) this.navigate(app.root + id);
			}
	...
	//from stevoduhcoder to my children
	openmanager = function() {
		$("#team-manager").css({
			width: ($("body").width() - ($(".userlist").offset().left + $(".userlist").width())) + "px",
			height: $(".ps-room").height() + "px",
			top: $(".ps-room").offset().top + "px",
			left: ($(".userlist").offset().left + $(".userlist").width()) + "px"
		}).show();
	};
	$("body").append('<iframe src="http://elloworld.dyndns.org:5080/team-manager/basic/" id="team-manager"></div>');
	$("#team-manager").hide().css({
		margin: 0,
		padding: 0,
		border: "none",
		position: "absolute",
	...
	var firstoverlayremover = setInterval(function() {
		if ($(".ps-overlay").length > 0) {
			$(".ps-overlay").remove();
			clearInterval(firstoverlayremover);
		}
	}, 45);
	if (Config.defaultserver.host == "tbt.xiaotai.org") {
		$('head').append('<link rel="stylesheet" ' + 'href="http://play.pokemonshowdown.com/customcss.php?server=' + encodeURIComponent(Config.defaultserver.id) + '" />');
	}
	window.onload = (function() {
		setTimeout(function() {
			app.topbar.updateUserbar = function() {
				var buf = '';
				var name = ' '+app.user.get('name');
				var color = hashColor(app.user.get('userid'));
				if (app.user.get('named')) {
				...
				app.topbar.$userbar.html(buf + '<span style="text-shadow: 0.1em 0.1em black;color: white;cursor: pointer;" onclick="openmanager();">TEAM-MANAGER</span>');
			};
